...
async function assignRoom(req, res) {
  ...
  try {
    ...
    if (device.houseId.toString() !== room.houseId.toString()) {
      return res
        .status(403)
        .json({ error: "Device belongs to a different house" });
    }
    const updated = await DeviceDao.assignRoom(body.id, body.roomId);
